...
function ProductPage( {addProductToCart, removeProductFromCart, currentCartProducts} ) {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:5000/reviews/product/${id}`);
  ...
  const handleLeaveReview = async () => {
    ...
    try {
      ...
      if (error.response) {
        console.log(`Error: ${error.response.data.message}`);
        setReviewErrorMessage('You must buy the product to leave a review.');
      } else {
        console.log('Network error. Please try again.');
      }
    }
          ...
          <div className='row'>
            <div className='col' style={{
              ...
              <ButtonRounded
                ...
                textSize={'1rem'}
                borderRadius={'0.5rem'}
                showBorder={isProductInCart() ? true : false}
                borderColor='#D9D9D9'
                backgroundColor={isProductInCart() ? '#FFFFFF' : '#11A36F'}
                accentColor={isProductInCart() ? '#4D4D4D' : '#FFFFFF'}
                textWeight={700}
              />
            </div>
          </div>
